    ...
    pub fn check(player: &mut Player) -> Vec<Achievement> {
        ...
        for ach in all {
            ...
            let completed = match ach.id {
                ...
                AchievementId::Lumberjack => player.stats.resources_gathered >= 100, // Simplified: assuming mostly wood if gathered
                AchievementId::Deforestation => player.stats.resources_gathered >= 1000,
                AchievementId::Miner => player.stats.resources_gathered >= 100, // Need to track distinct resource types in stats if strict
